...
export const loadPortalData = cache(async (): Promise<PortalData> => {
  ...
  const results = await Promise.all([
    ...
    supabase.from("property_settings").select("property_id, electric_rate, water_rate, water_billing_method, bill_day, due_day, late_fee, promptpay_id, account_name, invoice_note").eq("organization_id", organizationId),
    ...
    supabase.from("rent_invoices").select("id, property_id, room_id, lease_id, invoice_number, issued_at, due_at, subtotal, total, balance_due, status, note").eq("organization_id", organizationId).order("issued_at", { ascending: false }).limit(1000),
