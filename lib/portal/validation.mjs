...
export function validateLease(values) {
  const errors = {};
  for (const [key, message] of [["propertyId", "กรุณาเลือกหอพัก"], ["roomId", "กรุณาเลือกห้องพัก"], ["tenantId", "กรุณาเลือกผู้เช่า"]]) {
...
export function validateInvoice(values) {
  ...
  if (!/^\d{4}-\d{2}$/.test(text(values, "periodMonth"))) errors.periodMonth = "กรุณาเลือกรอบเดือน";
  if (!text(values, "invoiceNumber")) errors.invoiceNumber = "กรุณากรอกเลขที่ใบแจ้งหนี้";
