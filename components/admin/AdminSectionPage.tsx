...
export type AdminSearchParams = { saved?: string; error?: string; notice?: string; mode?: "role" | "user"; role?: string; organization?: string; user?: string };
...
export async function AdminSectionPage({ section, searchParams }: { section: AdminSection; searchParams: Promise<AdminSearchParams> }) {
  ...
  const [organizationsResult, organizationCountResult, profileCountResult, profilesResult, membershipsResult, subscriptionsResult, subscriptionPlansResult, adminsResult, rolesResult, permissionsResult, menusResult, auditsResult, trialRequestsResult, permissionActionsResult, menuActionsResult, granularRolePermissionsResult, userOverridesResult, registration] = await Promise.all([
    ...
    needsSubscriptions ? admin.from("subscriptions").select("organization_id, status, trial_ends_at, current_period_end, access_until, max_properties, max_rooms").order("created_at", { ascending: false }) : skipQuery(),
    needsSubscriptions ? admin.from("subscription_plans").select("*").order("sort_order", { ascending: true }) : skipQuery(),
  ...
  const subscriptionPlans = (subscriptionPlansResult.data && subscriptionPlansResult.data.length > 0)
    ? subscriptionPlansResult.data
    : [];
