...
import { saveRoleMenuActionsAction, saveUserMenuActionsAction, updateMembershipRoleAction } from "@/app/admin/actions";
...
export function RolePermissionMatrix({ menus, actions, roles, selectedRoleId }: { menus: MenuOption[]; actions: ActionOption[]; roles: RoleOption[]; selectedRoleId: string }) {
  ...
  return <div className="admin-permission-manager">
    <div className="admin-permission-toolbar"><label>Role<select value={role?.id ?? ""} onChange={(event) => changeLocation(`/admin?view=permissions&role=${event.target.value}`, dirty)}>{roles.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><label>คัดลอกสิทธิ์จาก Role<select disabled={locked} defaultValue="" onChange={(event) => { const source = roles.find((item) => item.id === event.target.value); if (source) { setValues(source.values); setDirty(true); } }}><option value="">เลือก Role</option>{roles.filter((item) => item.id !== role?.id).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label></div>
